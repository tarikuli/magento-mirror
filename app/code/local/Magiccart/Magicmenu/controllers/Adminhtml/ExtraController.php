 ...
 * @@Modify Date: 2015-12-16 21:48:20
 * @@Function:
 */
 ?>
<?php
class Magiccart_Magicmenu_Adminhtml_ExtraController extends Mage_Adminhtml_Controller_Action
{
	const NAVIGATION = 'magiccart/magicmenu_menus';	
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu(self::NAVIGATION)
			->_addBreadcrumb(Mage::helper('adminhtml')->__('Extra Menu Manager'), Mage::helper('adminhtml')->__('Extra Menu Manager'));
	...
	public function indexAction() {
		$this->_initAction()
			 ->_addContent($this->getLayout()->createBlock('magicmenu/adminhtml_extra'))
			 ->renderLayout();
	}
    ...
    public function gridAction()
    {
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('magicmenu/adminhtml_extra_grid')->toHtml()
        );
    }
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('extra_data', $model);
			...
			$this->_addContent($this->getLayout()->createBlock('magicmenu/adminhtml_extra_edit'))
				->_addLeft($this->getLayout()->createBlock('magicmenu/adminhtml_extra_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			if(!$data['link'] && !$data['ext_content']){
				Mage::getSingleton('adminhtml/session')->addError(Mage::helper('magicmenu')->__('Category custom already exist'));
				$this->_redirect('*/*/');
				return;
			}else {
				$data['extra'] = 1; 
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()
	    				   ->createBlock('magicmenu/adminhtml_extra_grid')
	        			   ->getCsv();
